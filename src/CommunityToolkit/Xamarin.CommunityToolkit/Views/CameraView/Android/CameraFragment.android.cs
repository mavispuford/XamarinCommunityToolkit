...
namespace Xamarin.CommunityToolkit.UI.Views
{
	class CameraFragment : Fragment, TextureView.ISurfaceTextureListener
	{
			...
			else
			{
				try
				{
					...
					previewImageReaderSize = ChooseOptimalSize(
						map.GetOutputSizes((int)ImageFormatType.Yuv420888) ?? throw new NullReferenceException(),
		...
		public void UpdateCaptureOptions()
		{
			...
			{
				CameraCaptureMode.Photo => CameraTemplate.StillCapture,
		...
		void SetupPreviewImageReader()
		{
			...
			previewReader = ImageReader.NewInstance(previewImageReaderSize?.Width ?? throw new NullReferenceException(), previewImageReaderSize.Height, ImageFormatType.Yuv420888, 1);
			...
			try
			{
				...
				if (previewSize != null && texture?.IsAvailable is true)
				{
					...
					else if (cameraTemplate == CameraTemplate.Preview)
					{
						...
						surfaces.Add(previewReader?.Surface ?? throw new NullReferenceException());
